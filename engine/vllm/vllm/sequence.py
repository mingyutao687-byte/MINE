...
class SequenceData:
    ...
    def __init__(
        ...
        self.kv_cache_need_restore: bool = False
        self.prompt_hash: Union[str, None] = None
    ...
    def get_hash_of_prompts_chxu(self) -> str:
        assert self.get_output_len() == 0
        if self.prompt_hash is not None:
            return self.prompt_hash
        tokens_list = self.get_token_ids()
        binary_bytes = struct.pack(f'{len(tokens_list)}i', *tokens_list)
        self.prompt_hash = hashlib.md5(binary_bytes).hexdigest()
        return self.prompt_hash

    def get_hash_of_all_tokens_chxu(self) -> str:
        tokens_list = self.get_token_ids()
        binary_bytes = struct.pack(f'{len(tokens_list)}i', *tokens_list)
        return hashlib.md5(binary_bytes).hexdigest()
    ...
    def restore_state_for_continue_decode_chxu(self) -> None:
        assert self.kv_cache_need_restore is False
        self.kv_cache_need_restore = True
        self._num_computed_tokens = self.get_len() - 1
        self._stage = SequenceStage.DECODE
...
class Sequence:
    ...
    def get_hash_of_prompts_chxu(self):
        return self.data.get_hash_of_prompts_chxu()
    ...
    def restore_state_for_continue_decode_chxu(self):
        assert self.status == SequenceStatus.WAITING
        self.status = SequenceStatus.RUNNING
        self.data.restore_state_for_continue_decode_chxu()
...
class SequenceGroup:
    ...
    def need_kv_cache_restore_chxu(self) -> bool:
        # We do something special here, force to enable decode
        if '-' in self.request_id:
            cur_model_id = self.request_id.split('-')[0]
        ...
        assert len(self.seqs_dict) == 1
        return kv_manager.have_request_kv_cache(self.request_id)
