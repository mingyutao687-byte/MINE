...
class GPUExecutor(ExecutorBase):
    ...
    def _init_executor(self) -> None:
        ...
        if envs.NO_MODEL_LOADING_AT_START:
            logger.warning('NO_MODEL_LOADING_AT_START set to True, will not load model at start!')
        else:
    ...
    def offload_model(self):
        self.driver_worker.offload_model()

    def scale_physical_kv_cache_chxu(self, new_num_blocks, src_dst_list):
        self.driver_worker.scale_physical_kv_cache_chxu(new_num_blocks, src_dst_list)

    def save_kv_cache_chxu(
            self,
            seq_group_metadata_list: list[SequenceGroupMetadata]):
        self.driver_worker.save_kv_cache_chxu(seq_group_metadata_list)
