...
class UncachedBlockAllocator(BlockAllocatorBase):
    ...
    def scale_logical_kv_cache_chxu(self, new_num_blocks):
        if new_num_blocks > self.num_blocks:
            for i in range(self.num_blocks, new_num_blocks):
                block = PhysicalTokenBlock(device=self.device,
                                           block_number=i,
                                           block_size=self.block_size,
                                           block_hash=-1,
            ...
            occupied_block_num = self.num_blocks - len(self.free_blocks)
            new_free_blocks = new_num_blocks - occupied_block_num
            self.free_blocks = self.free_blocks[: new_free_blocks]
            for idx, block in enumerate(self.free_blocks):
                block.block_number = occupied_block_num + idx
...
class BlockSpaceManagerV1(BlockSpaceManager):
    ...
    def scale_logical_kv_cache_chxu(self, new_num_blocks):
        logger.warning('scale logical kv-cache begin')
        src_dst_list = []
        occupied_block_num = self.gpu_allocator.get_num_total_blocks() - self.gpu_allocator.get_num_free_blocks()
        assert occupied_block_num <= new_num_blocks
        logger.warning(f'original num_blocks: {self.num_total_gpu_blocks}, now_use: {occupied_block_num}, '
                       f'new num_blocks: {new_num_blocks}')
        if new_num_blocks < self.num_total_gpu_blocks:
            # scale-in, need remapping
            dst_idx = 0
            for block_table_list in self.block_tables.values():
                for block_table in block_table_list:
                    if block_table.device == Device.GPU:
                        src_dst_list.append((block_table.block_number, dst_idx))
                        block_table.block_number = dst_idx
                        dst_idx += 1
            assert dst_idx == occupied_block_num
        self.gpu_allocator.scale_logical_kv_cache_chxu(new_num_blocks)
        self.num_total_gpu_blocks = new_num_blocks
        # logger.warning(f'logical remapping: {src_dst_list}')
        logger.warning('scale logical kv-cache end')
        return src_dst_list
