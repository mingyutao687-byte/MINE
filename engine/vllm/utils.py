...
def is_openvino() -> bool:
    """Removed — OpenVINO backend requires Intel AMX, not available on A10 system."""
    return False


@lru_cache(maxsize=None)
def is_neuron() -> bool:
    """Removed — AWS Neuron backend requires Trainium/Inferentia hardware."""
    return False


@lru_cache(maxsize=None)
def is_tpu() -> bool:
    """Removed — TPU backend requires Google TPU hardware."""
    return False


@lru_cache(maxsize=None)
def is_xpu() -> bool:
    """Removed — XPU backend requires Intel GPU hardware."""
    return False
    if not _import_ipex:
...
class CudaMemoryProfiler:
    ...
    def __enter__(self):
        ...
        logger.info(f'initial_memory: {self.initial_memory / (2 ** 20):.0f} MB')
        # This allows us to call methods of the context manager if needed
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.final_memory = self.current_memory_usage()
        logger.info(f'final_memory: {self.final_memory / (2 ** 20):.0f} MB')
