...
class DefaultModelLoader(BaseModelLoader):
    ...
    def load_model(self, *, model_config: ModelConfig,
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            logger.info(f'initializing model took {ed - st:.3f} s')
            st = time.perf_counter()
            # if len(model_config.cached_weights) == 0:
            #     logger.warning('loading cached weights...')
            #     for name, loaded_weight in self._get_weights_iterator(
            #             model_config.model, model_config.revision,
            #             fall_back_to_pt=getattr(model, "fall_back_to_pt_during_load", True)):
            #         model_config.cached_weights.append((name, loaded_weight))
            #
            #     logger.warning(f'cached_weights_list_length: {len(model_config.cached_weights)}')
            # model.load_weights(model_config.cached_weights, )
            model.load_weights(
                self._get_weights_iterator(model_config.model,
                                           ...
                                           fall_back_to_pt=getattr(
                                               ...
                                               True)), )
            torch.cuda.synchronize(device_config.device)
            ed = time.perf_counter()
            logger.info(f'loading weights took {ed - st:.3f} s')
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            with torch.device(device_config.device):
                model = _initialize_model(model_config, self.load_config,
                                          ...
                                          cache_config)
            ed = time.time()
            logger.info(f'initialize model took {ed - st:.3f} s')
            st = time.time()
            ...
            ed = time.time()
            logger.info(f'load model weights took {ed - st:.3f} s')
        return model.eval()
...
class ServerlessLLMLoader(BaseModelLoader):
    ...
    def save_model(
        ...
        for key, tensor in state_dict.items():
            ...
            total_size += state_dict[key].numel() * state_dict[key].element_size()
        logger.info(f"Total size of tensors: {total_size / 1024 / 1024:.2f} MB")
